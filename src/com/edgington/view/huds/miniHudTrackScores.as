package com.edgington.view.huds
{
	...
	public class miniHudTrackScores extends AbstractHud implements IAbstractHud
	{
		...
		public function setupVisuals():void
		{
			...
			if(SearchProxy.getInstance().isTournamentTrack){
				highscoresData.getTopXWithTrackkey(amountOfListings, SearchProxy.getInstance().tournamentID);
			}
			else{
					...
					switch(tabLabel)
					{
						...
						case tabs[1]:
							...
							if(highscoresFriends == null){
								if(SearchProxy.getInstance().isTournamentTrack){
									highscoresData.getFriendsScoresWithTrackkey(amountOfListings, SearchProxy.getInstance().tournamentID);
								}
								else{
