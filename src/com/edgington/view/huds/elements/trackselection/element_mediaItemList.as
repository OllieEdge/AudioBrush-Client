package com.edgington.view.huds.elements.trackselection
{
	...
	public class element_mediaItemList extends Sprite
	{
		...
		public var itemList:Vector.<AudioBrushMediaItemVO>;
		...
		public function element_mediaItemList(itemStrings:Vector.<AudioBrushMediaItemVO>, maximumItemsToShow:int, itemHeight:int, _width:int, itemHandlerSignal:Signal, selectedCollectionType:String)
		{
			super();
		...
		private function generateItems():void{
			...
			for(var i:int = 0; i < itemList.length; i++){
				...
				trackData = new TrackData(new <String>[itemList[i].trackDetails.album], trackArtwork);
				...
				txtTitle = TextFieldManager.createTextField(itemList[i].trackDetails.trackTitle, FONT_audiobrush_content_bold, Constants.DARK_FONT_COLOR, itemHeight*.35, false, TextFieldAutoSize.LEFT);
				...
				if(itemList[i].difficulty != ""){
					var difficultyRating:ui_difficulty_ratings_light = new ui_difficulty_ratings_light();
					difficultyRating.gotoAndStop(DifficultyTypes.difficultyStringIDToID(itemList[i].difficulty));
					difficultyRating.height = listing.height*.4;
					difficultyRating.scaleX = difficultyRating.scaleY;
					difficultyRating.x = listing.width - difficultyRating.width - difficultyRating.height;
					difficultyRating.y = listing.height*.5 - difficultyRating.height*.5;
					clip.addChild(difficultyRating);
				}
				...
				obj.title = txtTitle;
				if(itemList[i].difficulty != ""){
					obj.difficulty = difficultyRating
				}
				else{
					obj.difficulty = null;
				}
		...
		private function unColourItem(item:Object):void{
			...
			if(item.difficulty){
				item.clip.addChild(item.difficulty);
			}
			...
			{ 
				...
				switch(currentCollectionType){
					case CollectionType.ARTISTS:
						name1 = a.trackDetails.artist;
						name2 = b.trackDetails.artist;
						break;
					case CollectionType.ALBUMS:
						name1 = a.trackDetails.album;
						name2 = b.trackDetails.album;
						break;
					default:
						name1 = a.trackDetails.artist;
						name2 = b.trackDetails.artist;
